function [xtraj, ttraj, terminate_cond] = test_trajectoryGoalsmorethanBots(start, stop, map, path, vis,var)
...
controlhandle = @controllerAnu;
trajhandle    = @trajectory_generatorGoalsmorethanBots;
...
plotstart = cell2mat(start);
plotstart = plotstart';
goal = cell2mat(stop);
goal = goal';
intermediate = cell2mat(path');
intermediate = intermediate(1:2:end,:);
hold on
% plot3(plotstart(:,1),plotstart(:,2),plotstart(:,3),'bs','MarkerSize',8);
plot3(goal(:,1),goal(:,2),goal(:,3),'go','MarkerSize',6);
plot3(intermediate(:,1),intermediate(:,2),intermediate(:,3),'rs','MarkerSize',6);
legend('Final Goals','Intermediate Goals')
...
time_tol = 700;          % maximum simulation time
...
for iter = 1:max_iter
    ...
    for qn = 1:nquad
        ...
        if iter == 1
           ...
           lineHandles{qn} = line([plotstart(qn,1),goal(qn,1)], ...
                [plotstart(qn,2),goal(qn,2)],[plotstart(qn,3),goal(qn,3)]);
        ...
        if time<var.tf1
            set(lineHandles{qn},'XData',[desired_state.pos(1),intermediate(qn,1)], ...
                'YData',[desired_state.pos(2),intermediate(qn,2)], 'ZData',[desired_state.pos(3),intermediate(qn,3)])
        else
            set(lineHandles{qn},'XData',[desired_state.pos(1),goal(qn,1)], ...
                'YData',[desired_state.pos(2),goal(qn,2)], 'ZData',[desired_state.pos(3),goal(qn,3)])
        end
    end
